    ...
    <div class="flex-1">
        ...
        <div class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
            ...
            <div class="px-8 py-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between bg-white gap-4">
                <div>
                    <h3 class="font-extrabold text-[#0A1D37] text-lg">Daftar Guru Terdaftar</h3>
                    <p class="text-xs text-slate-400 mt-1">Total: {{ isset($gurus) ? $gurus->count() : 0 }} Guru</p>
                </div>
                ...
                <table class="w-full text-left whitespace-nowrap">
                    <thead class="bg-slate-50/50">
                        <tr>
                            ...
                            <th class="px-8 py-4 text-[10px] font-extrabold text-slate-400 uppercase tracking-widest cursor-pointer hover:bg-slate-100 transition group" onclick="window.location='{{ getSortUrl('mata_pelajaran', $sort ?? '', $order ?? '') }}'">
                                <div class="flex items-center gap-2">
                                    Mata Pelajaran
                                    {!! getSortIcon('mata_pelajaran', $sort ?? '', $order ?? '') !!}
                                </div>
                            </th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @forelse($gurus ?? [] as $guru)
                            <tr class="hover:bg-slate-50/80 transition duration-200 group">
                                ...
                                <td class="px-8 py-5">
                                    <div class="font-bold text-[#0A1D37] text-sm group-hover:text-blue-600 transition-colors">{{ $guru->nama }}</div>
                                </td>
                                <td class="px-8 py-5">
                                    <div class="text-slate-500 text-sm font-medium">{{ $guru->user->email ?? '-' }}</div>
                                </td>
                                <td class="px-8 py-5">
                                    <span class="bg-blue-50 text-blue-700 px-3 py-1 rounded-lg text-xs font-bold">{{ $guru->mata_pelajaran ?? 'Umum' }}</span>
                                </td>
                                <td class="px-8 py-5">
                                    <div class="flex items-center justify-center gap-3">
                                        <button type="button" onclick="openModalEdit('{{ $guru->id }}', '{{ addslashes($guru->nama) }}', '{{ $guru->user->email ?? '' }}', '{{ addslashes($guru->mata_pelajaran ?? '') }}')" class="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Guru">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </button>
                                        
                                        <button type="button" onclick="openModalHapus('{{ $guru->id }}', '{{ addslashes($guru->nama) }}')" class="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus Guru">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-8 py-12 text-center">
                                    ...
                                    <p class="text-slate-500 font-bold">Belum ada data guru.</p>
                                    <p class="text-xs text-slate-400 mt-1">Silakan klik "Tambah Guru" untuk memasukkan data baru.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="relative bg-white rounded-[2rem] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:max-w-lg w-full scale-95 opacity-0 duration-300" id="modalContentTambah">
            ...
            <form action="{{ route('admin.guru.store') }}" method="POST">
                ...
                <div class="p-6 sm:p-8 space-y-5">
                    <div>
                        <label class="block text-xs font-extrabold text-slate-700 mb-2 uppercase tracking-wide">Nama Lengkap Guru *</label>
                        <input type="text" name="nama" required placeholder="Contoh: Budi Santoso, S.Pd" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:bg-white focus:ring-2 focus:ring-[#0A1D37] focus:border-transparent outline-none transition-all">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-extrabold text-slate-700 mb-2 uppercase tracking-wide">Email Login *</label>
                            <input type="email" name="email" required placeholder="guru@smk.com" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:bg-white focus:ring-2 focus:ring-[#0A1D37] focus:border-transparent outline-none transition-all">
                        </div>
                        <div>
                    ...
                    <div>
                        <label class="block text-xs font-extrabold text-slate-700 mb-2 uppercase tracking-wide">Mata Pelajaran</label>
                        <input type="text" name="mata_pelajaran" placeholder="Contoh: Matematika" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:bg-white focus:ring-2 focus:ring-[#0A1D37] focus:border-transparent outline-none transition-all">
                    </div>
                </div>
                ...
                <div class="p-6 sm:p-8 space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="password" name="password" placeholder="Kosongkan jika tidak diubah" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:bg-white focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all placeholder:text-xs">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-extrabold text-slate-700 mb-2 uppercase tracking-wide">Mata Pelajaran</label>
                        <input type="text" id="edit_mapel" name="mata_pelajaran" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 text-sm focus:bg-white focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all">
                    </div>
                </div>
                <div class="bg-slate-50 px-6 py-4 flex justify-end gap-3 border-t border-slate-100">
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        <div class="relative bg-white rounded-[2rem] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:max-w-md w-full scale-95 opacity-0 duration-300" id="modalContentHapus">
            ...
            <div class="p-8 text-center">
                ...
                <h3 class="text-xl font-black text-[#0A1D37] mb-2">Hapus Data Guru?</h3>
                <p class="text-sm text-slate-500 font-medium leading-relaxed">
                    yakin hapus data <strong id="hapus_nama" class="text-red-600"></strong>? Data absen dan akses login guru akan ikut terhapus.
                </p>
            </div>
            ...
            <div class="bg-slate-50 px-6 py-5 flex justify-center gap-3 border-t border-slate-100">
                ...
                <form id="formHapusGuru" method="POST" class="w-full">
                    @csrf
                    @method('DELETE')
...
<script>
    ...
    function openModalEdit(id, nama, email, mapel) {
        document.getElementById('formEditGuru').action = baseUrl + '/' + id;
    ...
    function openModalHapus(id, nama) {
        document.getElementById('formHapusGuru').action = baseUrl + '/' + id;
